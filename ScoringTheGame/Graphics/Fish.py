...
class leftPiranha(Fish):
    ...
    def reset_pos(self):
        """ Reset position to the right of the screen, at a random y location.
        Called by update() or the main program loop if there is a collision.
        """
    ...
    def update(self):
        self.fishType = "leftPiranha"
        self.image = leftPiranha_image
        self.rect.x -= 20
        self.rect.y += 1
        if self.rect.y < 0:
            self.reset_pos()
...
leftPiranha = leftPiranha(leftPiranha_image)
all_sprites_list.add(leftPiranha)
